...
$sql = "INSERT into ANUNCIOS (razon_soc, cif, direccion, latitud, longitud, telefono, email, descripcion,  imagen, fecha, usuario_id, categoria_id) VALUES (:razon, :cif, :direccion, :lat, :lng, :telefono, :email, :descripcion, :imagen, :fecha, :usuario_id, :categoria_id)";

$query = $con->prepare($sql);
$query->execute(array(":razon"=>$razon, ":cif"=>$cif,":direccion"=>$direccion,":latitud"=>$lat,":lng"=>$lng,":telefono"=>$telefono,":email"=>$email,":descripcion"=>$descripcion,":imagen"=>$imagen,":fecha"=>$fecha,":user_id"=>$id_usuario, ":categoria"=>$id_categoria));


//Actualizamos los registros de nuestra BD

 $sql = "SELECT a.id_anuncio, a.razon_soc, a.cif, a.direccion, a.telefono, a.email, a.descripcion, a.imagen, a.total_votos, c.nombre_cat FROM anuncios a INNER JOIN categorias c on a.categoria_id = c.id_categoria WHERE a.usuario_id = '$id_usuario'";
 $query = $con->prepare($sql);
 $query->execute();
    ...
    while ($resultado = $query->fetch(PDO::FETCH_ASSOC)){                  
        echo '<table class="table table-striped table-condensed table-hover">
                ...
                <tr>
                    ...
                    <td><a data-container="body" data-toggle="popover" data-placement="bottom" data-content="'.$resultado['descripcion'].'"><i class="fa fa-eye fa-2x" aria-hidden="true"></i></a></td>
                     <td><a data-container="body" data-toggle="popover" data-placement="bottom" data-content="<ul><li>Dirección: '.$resultado['direccion'].'</li>
                             ...
                             </ul>"><i class="fa fa-eye fa-2x" aria-hidden="true"></i></a></td>
